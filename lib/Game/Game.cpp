...
Game::Game() : score(0), trs_score(0), scoreCount(0) {}
...
void Game::saveScore() {
  ...
  sortScores();  // сортируем по убыванию
}
bool Game::saveScores() {
  File file = LittleFS.open("/scores.txt", "w");
  if (!file) {
    Serial.println("Failed to open scores file for writing");
    return false;
  }
  ...
  file.close();
  Serial.println("Scores saved to file.");
  return true;
}

bool Game::loadScores() {
  File file = LittleFS.open("/scores.txt", "r");
  if (!file) {
    Serial.println("Failed to open scores file for reading");
    return false;
  }
...
bool Game::outOfTime()
{
  return (elapsedTime/1000) > 60; // This should be a constant
}
